...
const DRAFT_KEY = 'gojos_booking_draft';
  ...
  return (
    <div className="flex flex-col gap-6 py-8 px-4 md:px-8 max-w-4xl mx-auto w-full text-left">
      ...
      <div className="flex flex-col gap-2">
        ...
        <p className="text-slate-400 text-sm">Follow the steps to secure your court booking at Gojo's Bistro.</p>
      ...
      {/* STEP 1: Details */}
      {step === 1 && (
        <Card className="glass border-white/5 shadow-2xl bg-slate-950/40"><form onSubmit={handleStep1Submit}><CardContent className="pt-6 flex flex-col gap-5">
          <div className="flex items-center gap-2 mb-2 font-display font-bold text-white text-lg"><User size={18} className="text-emerald-400" /> Basic Information</div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="flex flex-col gap-1.5">
              ...
              <Input
                ...
                className="bg-slate-950/50 border-white/10 text-white placeholder:text-slate-600"
              />
            </div>
          ...
          <Button type="submit" className="w-full sm:w-auto self-end py-6 bg-emerald-500 hover:bg-emerald-600 text-slate-950 font-bold rounded-xl mt-4">Choose Slot <ChevronRight size={16} /></Button>
        </CardContent></form></Card>
      )}

      {/* STEP 2: Time Selection */}
      {step === 2 && (
        <Card className="glass border-white/5 shadow-2xl bg-slate-950/40"><CardContent className="pt-6 flex flex-col gap-5">
          <div className="flex items-center justify-between">
            <div className="font-display font-bold text-white text-lg flex items-center gap-2"><Calendar size={18} className="text-emerald-400" /> Select Schedule Slot</div>
            <Button variant="ghost" size="sm" onClick={prevStep} className="text-xs text-slate-400 hover:text-white"><ChevronLeft size={14} /> Back</Button>
          </div>
      ...
      {step === 3 && (
        <Card className="glass border-white/5 shadow-2xl bg-slate-950/40"><form onSubmit={e => { e.preventDefault(); nextStep(); }}><CardContent className="pt-6 flex flex-col gap-5">
          <div className="flex items-center justify-between">
            <div className="font-display font-bold text-white text-lg flex items-center gap-2"><ShoppingCart size={18} className="text-emerald-400" /> Customize Equipment & Coaching</div>
            <Button variant="ghost" size="sm" onClick={prevStep} className="text-xs text-slate-400 hover:text-white"><ChevronLeft size={14} /> Back</Button>
          </div>
      ...
      {step === 4 && (
        <Card className="glass border-white/5 shadow-2xl bg-slate-950/40"><CardContent className="pt-6 flex flex-col gap-5">
          <div className="flex items-center justify-between">
            <div className="font-display font-bold text-white text-lg flex items-center gap-2"><ShoppingCart size={18} className="text-emerald-400" /> Review Booking Hold</div>
            <Button variant="ghost" size="sm" onClick={prevStep} className="text-xs text-slate-400 hover:text-white"><ChevronLeft size={14} /> Back</Button>
          </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <Button
              ...
              disabled={loading}
              className="w-full py-6 bg-emerald-500 hover:bg-emerald-600 text-slate-950 font-bold rounded-xl mt-2"
            >
              {loading ? (
      ...
      {/* STEP 5: Payment Form */}
      {step === 5 && (
        <Card className="glass border-white/5 shadow-2xl bg-slate-950/40"><CardContent className="pt-6 flex flex-col gap-6">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex flex-col text-sm">
                      ...
                      <p className="text-slate-500 text-[11px] mt-2">
                        I accept the GOJO'S BISTRO waiver policy. You must read and accept the waiver before proceeding.{' '}
      ...
      <Dialog open={!!downgradeDialog} onOpenChange={(o) => { if (!o) setDowngradeDialog(null) }}>
        <DialogContent className="glass border-white/15 bg-slate-950/80 rounded-2xl sm:max-w-sm">
          {downgradeDialog && (
            <>
              <DialogHeader><DialogTitle className="flex items-center gap-2.5 text-amber-400"><AlertTriangle size={24} /> <span className="font-display text-white">Partial Slot Available</span></DialogTitle></DialogHeader>
              ...
              <DialogFooter className="mt-2"><Button variant="outline" onClick={() => setDowngradeDialog(null)} className="text-slate-400 border-white/10">Cancel</Button><Button onClick={confirmDowngrade} className="bg-emerald-500 hover:bg-emerald-600 text-slate-950">Downgrade & Continue</Button></DialogFooter>
            </>
          )}
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={waiverModalOpen} onOpenChange={setWaiverModalOpen}>
        <DialogContent className="glass border-white/15 bg-slate-950/80 rounded-2xl sm:max-w-2xl max-h-[85vh] overflow-y-auto">
          <DialogHeader><DialogTitle className="font-display font-bold text-xl text-white">Waiver Policy</DialogTitle></DialogHeader>
            <div className="overflow-y-auto pr-2 flex flex-col gap-4 text-xs text-slate-300 leading-relaxed no-scrollbar text-left">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <p>I understand that participation in pickleball and related activities at GOJO'S BISTRO involves inherent risks that cannot be fully eliminated, regardless of the care taken to avoid injuries. These risks include, but are not limited to:</p>
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <p>In consideration of being permitted to use the facilities, equipment, and services provided by GOJO'S BISTRO, I, for myself and my heirs, executors, administrators, and assigns, hereby release, waive, discharge, and covenant not to sue GOJO'S BISTRO, its owners, officers, employees, contractors, volunteers, sponsors, or agents for any and all liability, claims, demands, actions, or causes of action arising out of or related to any loss, damage, or injury, including death, that may occur while participating in facility activities.</p>
              </div>

              <div className="flex flex-col gap-1.5">
                <h4 className="font-semibold text-emerald-400 uppercase tracking-wider text-[10px]">Indemnification and Hold Harmless</h4>
                <p>I agree to indemnify and hold harmless GOJO'S BISTRO, its officers, employees, and agents from any claims, damages, or costs (including attorney’s fees) arising from my participation or conduct while at the facility, whether caused by negligence or otherwise.</p>
              </div>
              ...
              <div className="flex flex-col gap-2 mt-2 bg-slate-900/50 p-3 rounded-lg border border-white/5">
                ...
                <div className="flex items-start gap-2">
                  ...
                  <label htmlFor="likenessCheckbox" className="text-xs text-slate-300 cursor-pointer leading-tight">
                    I grant GOJO'S BISTRO permission to use photographs, video recordings, or other media of my participation for promotional, educational, or informational purposes without compensation or restriction.
                  </label>
                </div>
              </div>
            ...
            <DialogFooter className="pt-2 border-t border-white/5 mt-2">
              <Button onClick={() => setWaiverModalOpen(false)} className="bg-emerald-500 hover:bg-emerald-600 text-slate-950 font-bold">Close & Return</Button>
            </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={policyModalOpen} onOpenChange={setPolicyModalOpen}>
        <DialogContent className="glass border-white/15 bg-slate-950/80 rounded-2xl sm:max-w-md">
          <DialogHeader><DialogTitle className="font-display font-bold text-xl text-white">Refund & Cancellation Policy</DialogTitle></DialogHeader>
            ...
            <DialogFooter className="pt-2 border-t border-white/5 mt-2">
              <Button onClick={() => setPolicyModalOpen(false)} className="bg-emerald-500 hover:bg-emerald-600 text-slate-950 font-bold">Understood</Button>
            </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
