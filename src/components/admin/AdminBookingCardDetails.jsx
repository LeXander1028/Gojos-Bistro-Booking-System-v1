  ...
  return (
    <div className="glass border border-white/5 rounded-2xl p-5 flex flex-col gap-4 text-left transition-all duration-300 hover:border-white/10 hover:shadow-xl">
      ...
      {showCancelModal && createPortal(
        ...
        >
          <div 
            className="glass border border-rose-500/30 rounded-2xl w-full max-w-sm p-6 text-center shadow-[0_0_30px_rgba(225,29,72,0.15)]"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="w-12 h-12 rounded-full bg-rose-500/10 text-rose-400 flex items-center justify-center mx-auto mb-4">
              <ShieldAlert size={24} />
            </div>
            <h3 className="font-display font-bold text-white text-lg mb-2">Cancel Booking</h3>
            <p className="text-sm text-slate-400 mb-4">
              Are you sure you want to cancel this booking for <strong className="text-white">{parsedNotes.bookerName || 'Guest'}</strong>?
            </p>
            <div className="flex flex-col gap-3 text-left mb-6">
              <label className="text-xs text-slate-400 font-semibold">Reason for rejection (optional)</label>
              <input
                type="text"
                placeholder="e.g. Unpaid, Double Booking..."
                value={cancelReason}
                onChange={e => setCancelReason(e.target.value)}
                className="w-full bg-slate-950 border border-white/5 focus:border-rose-500 rounded-xl py-2 px-3 text-xs text-white placeholder-slate-700 focus:outline-none"
              />
            </div>
            <div className="flex flex-col gap-3">
              <button
                ...
                className="w-full py-3 bg-rose-500 hover:bg-rose-600 text-slate-950 font-bold text-sm rounded-xl transition-all shadow-[0_0_15px_rgba(225,29,72,0.3)]"
              >
