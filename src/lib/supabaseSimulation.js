...
const STORAGE_KEY = 'gojos_simulation_db';
    ...
    payment_methods: [
      {
        ...
        name: 'GCash',
        account_name: 'Gojo\'s Bistro Prenza (0917-123-4567)',
      ...
      },
      {
        ...
        name: 'GoTyme Bank',
        account_name: 'Gojo\'s Bistro Sports Center (Account: 1234-5678-9012)',
...
const storedSession = localStorage.getItem('gojos_simulation_session');
if (storedSession) {
  try {
    currentSession = JSON.parse(storedSession);
  } catch (e) {
    localStorage.removeItem('gojos_simulation_session');
  }
}
...
export const supabaseSimulation = {
  auth: {
    signUp: async ({ email, password, options }) => {
      ...
      localStorage.setItem('gojos_simulation_session', JSON.stringify(session));
      notifyAuthChange();
    ...
    signInWithPassword: async ({ email, password }) => {
      ...
      if (email === 'admin@hqpickle.ph' || email === 'admin@gojosbistro.com') {
        profile = db.profiles.find(p => p.role === 'admin');
      } else if (email === 'player@hqpickle.ph' || email === 'player@gojosbistro.com') {
